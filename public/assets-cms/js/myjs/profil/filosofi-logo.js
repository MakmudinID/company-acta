...
jQuery(document).ready(function() {
    ...
    table = $('#table').DataTable({
        "ajax": {
            "url": base_url + "/adminprofil/filosofi_logo_",
            "type": "POST",
            data: function(d){
                d[csrfName] = csrfHash;
            }
    ...
    $(document).on('click', '.edit', function() {
        ...
        $('#modal-default').modal('show');
        $('.update').text('Update');
        $('.modal-title').text('Edit Filosofi Logo');
    ...
    $(document).on('click', '.delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: base_url + "/adminprofil/delete_filosofi_logo/" + id,
                    type: "GET",
                    dataType: "JSON",
    ...
    $.validator.setDefaults({
        submitHandler: function() {
            var url;
            if (save_method == 'add') {
                url = base_url + "/adminprofil/create_filosofi_logo";
            } else {
                url = base_url + "/adminprofil/update_filosofi_logo";
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.result != true) {
                        Swal.fire({
                            title: data.title,
                            html: 'filosofi_logo <br>' + data.status,
                            icon: data.icon,
                            timer: 3000,
                            showCancelButton: false,
                            showConfirmButton: false,
